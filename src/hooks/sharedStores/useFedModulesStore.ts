...
// Sort key types
const sortKeys = ['id', 'manifestLocation', 'moduleCount', 'ssoScopesCount'] as const;
...
export const isSetSortPayload = (event: EventType, payload: unknown): payload is SetSortPayload => {
  if (
    event !== 'SET_SORT' ||
    typeof payload !== 'object' ||
    payload === null ||
...
const applyFiltersAndSort = (
  ...
  if (
    ...
    filtered = Object.keys(data).reduce((acc, key) => {
        ...
        const moduleMatch = entry.modules
          ? entry.modules.some(
              (mod) =>
                ...
                mod.routes.some((route) => route.pathname.toLowerCase().includes(searchLower))
            )
          : false;
      ...
      if (filterConfig.ssoScopeFilter.length > 0) {
        ...
        const ssoScopes = entry.config?.ssoScopes || entry.moduleConfig?.ssoScopes || [];

        // Check if any of the entry's scopes match any of the selected filter scopes
        const scopeMatch = ssoScopes.some((scope) => filterConfig.ssoScopeFilter.includes(scope));
      ...
      if (filterConfig.moduleFilter) {
        const moduleMatch = entry.modules
          ? entry.modules.some(
              (mod) =>
                mod.id.toLowerCase().includes(filterConfig.moduleFilter.toLowerCase()) ||
                mod.module.toLowerCase().includes(filterConfig.moduleFilter.toLowerCase())
            )
          : false;
  ...
  // Apply sorting
  if (sortConfig.key) {
    const sortedEntries = Object.entries(filtered).sort(([keyA, entryA], [keyB, entryB]) => {
...
const handleEvents = (
  ...
  switch (event) {
    ...
    case 'FETCH_SUCCESS':
      ...
      return {
        ...newState,
        filteredData: applyFiltersAndSort(payload.data, newState.filterConfig, newState.sortConfig)
      };
    ...
    case 'SET_FILTER':
      ...
      return {
        ...
        filteredData: applyFiltersAndSort(prevState.data, updatedFilterConfig, prevState.sortConfig)
      };
    ...
    case 'SET_SORT':
      ...
      return {
        ...
        filteredData: applyFiltersAndSort(prevState.data, prevState.filterConfig, updatedSortConfig)
      };
    ...
    case 'CLEAR_FILTERS':
      ...
      return {
        ...
        filteredData: applyFiltersAndSort(prevState.data, clearedFilterConfig, prevState.sortConfig)
      };
...
let store: ReturnType<typeof createSharedStore<FedModulesState, typeof EVENTS>> | null = null;
...
export const useFedModulesStore = () => {
  ...
  const fetchFedModules = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/chrome-service/v1/static/fed-modules-generated.json');
      if (!response.ok) {
      ...
      store.updateState('FETCH_ERROR', {
        error: error instanceof Error ? error.message : 'Failed to fetch fed modules'
      });
    }
  }, [store]);
  ...
  const toggleSort = useCallback(
    (key: SortKey) => {
      const currentDirection = state.sortConfig.key === key ? state.sortConfig.direction : 'asc';
  ...
  return {
    ...
    filteredCount: state.filteredData ? Object.keys(state.filteredData).length : 0
  };
};
